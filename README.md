...
Send DELETE /api/claims/{id} with a GUID.

Service deletes the claim and publishes ClaimDeletedEvent.

Background queue handles the event asynchronously and saves an audit record.

API immediately returns 204 No Content.
...
Event-Driven: Important operations publish events, decoupled from logging.

Asynchronous Logging: Background queue keeps API fast and responsive.

Unit Testing: Services are testable with Moq.

Scalable & Maintainable: Can be extended to multiple services or microservices easily.

Azure Service Bus Ready: The architecture allows events to be pushed to Azure Service Bus, enabling other microservices to react without modifying this API.
